...
public class ViEAndroidGLES20 extends GLSurfaceView
    ...
    private static class ContextFactory implements GLSurfaceView.EGLContextFactory {
        ...
        public EGLContext createContext(EGL10 egl, EGLDisplay display, EGLConfig eglConfig) {
        	if(ViESurfaceRenderer.DEBUG){
        		ECLog4Util.w(TAG, "creating OpenGL ES 2.0 context");
        	}
    ...
    private static void checkEglError(String prompt, EGL10 egl) {
        ...
        while ((error = egl.eglGetError()) != EGL10.EGL_SUCCESS) {
        	if(ViESurfaceRenderer.DEBUG){
        		ECLog4Util.e(TAG, String.format("%s: EGL error: 0x%x", prompt, error));
        	}
        }
    }
    ...
    private static class ConfigChooser implements GLSurfaceView.EGLConfigChooser {
        ...
        private void printConfigs(EGL10 egl, EGLDisplay display,
            ...
            if(ViESurfaceRenderer.DEBUG){
        		ECLog4Util.w(TAG, String.format("%d configurations", numConfigs));
        	}
            for (int i = 0; i < numConfigs; i++) {
            	if(ViESurfaceRenderer.DEBUG){
            		ECLog4Util.w(TAG, String.format("Configuration %d:\n", i));
            	}
        ...
        private void printConfig(EGL10 egl, EGLDisplay display,
            ...
            for (int i = 0; i < attributes.length; i++) {
                ...
                if (egl.eglGetConfigAttrib(display, config, attribute, value)) {
                	if(ViESurfaceRenderer.DEBUG){
                		ECLog4Util.w(TAG, String.format("  %s: %d\n", name, value[0]));
                	}
                } else {
